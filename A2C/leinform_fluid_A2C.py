...
os.chdir(os.path.dirname("/home/mamitsu/OpenFOAM/mamitsu-6/run/optiair_room/A2C/"))
os.getcwd()
# set directory
# 試す用
...
class Aircond:
    ...
    def __init__(self, CASE, stride=10,end=3000,
        ...
        self.ACTION_SPEED = np.array([0.1,0.2,0.3,0.5])
    ...
    def step(self, action, sqb=0.0):
        ...
        if self.done:
            ...
            self.startTime = self.controlDict['startTime']
            self.endTime = self.controlDict['endTime']
...
# 初期状態の開始
obs = resets(Envs,initial_T=initial_T,is_wall=is_wall,is_flux=is_flux, \
            wall_boundary=wall_boundary, flux_boundary=flux_boundary, \
            wall_condition=wall_condition,inner=inner,base=base, \
            term=term,domain=domain, pattern=pattern,kind=kind, \
            flux_pattern=flux_pattern,flux_interval=flux_interval)
