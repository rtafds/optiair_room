...
class Aircond:
    ...
    def __init__(self, CASE, stride=100,end=3000,
        ...
        self.observation_space = self.make_observation(self.CASE.initialDir(), celsius=True, temp_only=temp_only)
    ...
    def step(self, action, sqb=0.1):
        ...
        if self.done or not self.runOK:
            ...
            if not self.runOK:
                self.reward = self.reward - 1000*self.stride
...
# resetsのパラメータの全て
initial_T="random"  # 指定 or random or mean, meanはis_wall=Trueのみ
is_wall=True  # True or False or random。randomはwall_conditionがterm,domain,patternの設定でランダム
is_flux=True  # True or False
wall_boundary = [['floor','sun'], ['ceiling'], ['sWall', 'glass1'],['nWall','glass2']]
flux_boundary = ['heatsource1','heatsource2']
wall_condition=[['Shade','Shade','S','N'],[11,12,13]]  # condition
inner=['Shade']  # *0.3にする壁
...
for j in tqdm(range(PLAY_STEP)):
    ...
    for step in range(NUM_ADVANCED_STEP):
        ...
        print(Env_play[0].present_time)
        print(Env_play[0].end)
        print(Env_play[1].present_time)
        print(Env_play[1].end)
